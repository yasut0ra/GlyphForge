...
class LocalScreenshotEvaluator(ScreenshotEvaluator):
    ...
    def evaluate(
        ...
        # Keep the AA canvas aspect ratio: regions are normalized to the actual
        # text grid, not to a padded square with unrelated margins.
        scale = 384 / max(screenshot.size)
        size = tuple(max(3, round(side * scale)) for side in screenshot.size)
        rendered = _ink_canvas(screenshot, size)
        target = _ink_canvas(reference, size)
        ...
        readability = float(np.clip(1.0 - abs(coverage - target_coverage) / max(target_coverage, 0.05), 0.0, 1.0))
        ...
        for row in range(3):
            for col in range(3):
                y0, y1 = row * size[1] // 3, (row + 1) * size[1] // 3
                x0, x1 = col * size[0] // 3, (col + 1) * size[0] // 3
        ...
        regions = [
            ...
            for value, row, col in worst if value > 1e-6
        ]
        return ScreenshotAssessment(
            ...
            summary=f"{plan.subject}のCanvas描画を構造差で評価しました（対象の認識精度ではありません）。",
